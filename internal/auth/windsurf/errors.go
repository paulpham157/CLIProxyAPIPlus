...
func (e *AuthenticationError) Error() string {
	if e.Err != nil {
		return fmt.Sprintf("windsurf auth: %s: %v", e.Message, e.Err)
	}
	return fmt.Sprintf("windsurf auth: %s", e.Message)
}
...
// OAuthError represents an OAuth-specific error from the Windsurf API.
...
func (e *OAuthError) Error() string {
	if e.ErrorDescription != "" {
		return fmt.Sprintf("windsurf oauth: %s: %s (status %d)", e.Error, e.ErrorDescription, e.StatusCode)
	}
	return fmt.Sprintf("windsurf oauth: %s (status %d)", e.Error, e.StatusCode)
}
