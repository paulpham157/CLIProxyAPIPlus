...
const (
	// windsurfClientID is Windsurf's OAuth client ID.
	windsurfClientID = "windsurf-cli"
	// windsurfDeviceCodeURL is the endpoint for requesting device codes.
	windsurfDeviceCodeURL = "https://auth.codeium.com/oauth/device/code"
	// windsurfTokenURL is the endpoint for exchanging device codes for tokens.
	windsurfTokenURL = "https://auth.codeium.com/oauth/token"
	// windsurfUserInfoURL is the endpoint for fetching user information.
	windsurfUserInfoURL = "https://api.codeium.com/v1/user"
...
// DeviceFlowClient handles the OAuth2 device flow for Windsurf.
...
// RequestDeviceCode initiates the device flow by requesting a device code from Windsurf.
func (c *DeviceFlowClient) RequestDeviceCode(ctx context.Context) (*DeviceCodeResponse, error) {
	data := url.Values{}
	data.Set("client_id", windsurfClientID)
	data.Set("scope", "openid profile email")

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, windsurfDeviceCodeURL, strings.NewReader(data.Encode()))
	if err != nil {
	...
	defer func() {
		if errClose := resp.Body.Close(); errClose != nil {
			log.Errorf("windsurf device code: close body error: %v", errClose)
		}
	}()
...
func (c *DeviceFlowClient) exchangeDeviceCode(ctx context.Context, deviceCode string) (*WindsurfTokenData, error) {
	data := url.Values{}
	data.Set("client_id", windsurfClientID)
	data.Set("device_code", deviceCode)
	data.Set("grant_type", "urn:ietf:params:oauth:grant-type:device_code")

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, windsurfTokenURL, strings.NewReader(data.Encode()))
	if err != nil {
	...
	defer func() {
		if errClose := resp.Body.Close(); errClose != nil {
			log.Errorf("windsurf token exchange: close body error: %v", errClose)
		}
	}()
...
// FetchUserInfo retrieves the user information for the authenticated user.
func (c *DeviceFlowClient) FetchUserInfo(ctx context.Context, accessToken string) (string, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, windsurfUserInfoURL, nil)
	if err != nil {
	...
	defer func() {
		if errClose := resp.Body.Close(); errClose != nil {
			log.Errorf("windsurf user info: close body error: %v", errClose)
		}
	}()
	...
	return "", NewAuthenticationError(ErrUserInfoFailed, fmt.Errorf("empty user info"))
}
