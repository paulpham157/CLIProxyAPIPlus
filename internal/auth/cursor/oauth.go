...
const (
	// cursorClientID is Cursor's OAuth client ID.
	cursorClientID = "cursor-cli"
	// cursorDeviceCodeURL is the endpoint for requesting device codes.
	cursorDeviceCodeURL = "https://authenticator.cursor.sh/oauth/device/code"
	// cursorTokenURL is the endpoint for exchanging device codes for tokens.
	cursorTokenURL = "https://authenticator.cursor.sh/oauth/token"
	// cursorUserInfoURL is the endpoint for fetching user information.
	cursorUserInfoURL = "https://api.cursor.sh/v1/user"
...
// DeviceFlowClient handles the OAuth2 device flow for Cursor.
...
// RequestDeviceCode initiates the device flow by requesting a device code from Cursor.
func (c *DeviceFlowClient) RequestDeviceCode(ctx context.Context) (*DeviceCodeResponse, error) {
	data := url.Values{}
	data.Set("client_id", cursorClientID)
	data.Set("scope", "user:email")

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, cursorDeviceCodeURL, strings.NewReader(data.Encode()))
	if err != nil {
	...
	defer func() {
		if errClose := resp.Body.Close(); errClose != nil {
			log.Errorf("cursor device code: close body error: %v", errClose)
		}
	}()
...
func (c *DeviceFlowClient) exchangeDeviceCode(ctx context.Context, deviceCode string) (*CursorTokenData, error) {
	data := url.Values{}
	data.Set("client_id", cursorClientID)
	data.Set("device_code", deviceCode)
	data.Set("grant_type", "urn:ietf:params:oauth:grant-type:device_code")

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, cursorTokenURL, strings.NewReader(data.Encode()))
	if err != nil {
	...
	defer func() {
		if errClose := resp.Body.Close(); errClose != nil {
			log.Errorf("cursor token exchange: close body error: %v", errClose)
		}
	}()
